...
from condstylegan.utils import add_noise, instance_norm, ELRConv2D, ELRDense
...
class ConditionalStyleGAN():
    ...
    def train_step(self, imgs, conditioning, fade):
        # Generate noise from normal distribution
        noise = tf.random_normal([self.batch_size, self.z_dim])

        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            generated_imgs = self.generator(inputs=[noise, conditioning, fade, tf.ones((self.batch_size, 1))],
                                            training=True)
            ...
            disc_loss = self.discriminator_loss(imgs, generated_imgs, real_disc, generated_disc)
    ...
    def train(self, data, img_size, start_size, epochs, lr, save_dir, b1, b2, save_int, num_out, **kwargs):
        ...
        for resolution in range(resolutions):
            ...
            for epoch in range(epochs):
                for imgs, conditioning in data:
                    if resolution < resolutions - 1:
                        # Set floor for images such that min value is 1/2 of images size
                        pool_factor = 2**(resolutions - resolution - 1)
                        imgs = kl.AveragePooling2D(pool_factor, padding='same')(imgs)
                    fade = epoch/(epochs//2)
                    if fade == 1:
                        self.fade = False
                    self.train_step(imgs=tf.cast(imgs, tf.float32),
                                    conditioning=conditioning,
                                    fade=tf.constant(fade, shape=(self.batch_size, 1), dtype=tf.float32))

                # Save every n intervals
                if (epoch + 1) % save_int == 0:
                    conditioning = tf.SparseTensor(indices=[random.randint(0, self.num_classes - 1)],
                                                   values=[1], dense_shape=[self.num_classes])
                    self.generate(epoch + 1, save_dir, num_out, conditioning)
        ...
        conditioning = tf.SparseTensor(indices=[random.randint(0, self.num_classes - 1)],
                                       values=[1], dense_shape=[self.num_classes])
        self.generate(epochs, save_dir, num_out, conditioning)
    ...
    def generator_model(self, out_size, start_size=8, start_filters=512):
        ...
        # Mapping network
        w = tf.concat((z, conditioning)) # Concatenate noise input with condition labels
        for layer in range(self.map_layers):
            w = ELRDense(start_filters, lrmul=0.01, name='dense_map' + '_' + str(layer))
        ...
        for resolution in range(conv_loop):
            ...
            if resolution == 0:  # 4x4
                with tf.variable_scope('Constant'):
                    x = kl.Dense(4 * 4 * filters)(constant)
                    ...
                    x = instance_norm(x)
                    style_mul = ELRDense(units=filters, name='dense_' + str(resolution) + '_00')(w)
                    style_add = ELRDense(units=filters, bias_initializer='Ones',
                                         name='dense_' + str(resolution) + '_01')(w)
                    x = kl.Multiply()([x, style_mul])
                    x = kl.Add()([x, style_add])

                # Single convolution.
                with tf.variable_scope('Conv_' + str(resolution) + '_0'):
                    x = ELRConv2D(filters=filters, kernel_size=3, padding='same',
        ...
        convert_to_image = kl.Conv2DTranspose(filters=3, kernel_size=1, strides=1, padding='same',
    ...
    def discriminator_loss(self, real_imgs, generated_imgs, real_disc, generated_disc, gp_lambda=10):
        # WGAN-GP loss: https://arxiv.org/pdf/1704.00028.pdf
    ...
    def generate(self, epoch, save_dir, num_out, conditioning):
        ...
        generated_imgs = self.generator(
            inputs=[gen_noise, conditioning, tf.constant(1, shape=[num_out, 1]), tf.ones((self.batch_size, 1))],
            training=False)
